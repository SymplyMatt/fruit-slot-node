...
export default async function getUserBookingOverview ( req : AuthenticatedRequest, res : Response) {
    try {
        let document : Model<BookingModel> = Booking;
        let allItems : Array<BookingModel> = await document.where("createdBy").equals(req.query.userId);
