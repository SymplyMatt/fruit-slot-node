...
export default async function getAllAdmins ( req : AuthenticatedRequest, res : Response) {
    try {
        let document : Model<UserModel> = User;
        let allItems : Array<UserModel> = await document.find();
        let allAdmins : Array<UserModel> = allItems.filter( item => item.roles.includes(2001) || item.roles.includes(2002));
        let array: Array<any> =  allAdmins.map(item => sendUserInfo(item));
