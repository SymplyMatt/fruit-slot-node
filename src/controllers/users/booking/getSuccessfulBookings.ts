...
export default async function getAllSuccessfulBookings ( req : AuthenticatedRequest, res : Response) {
    try {
        let document : Model<BookingModel> = Booking;
        let allItems : Array<BookingModel> = await document.where('createdBy').equals(req.user).where('status').equals('successful');
