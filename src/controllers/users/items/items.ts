...
export default async function getAllItems ( req : AuthenticatedRequest, res : Response) {
    try {
        let document : Model<AcceptableItemModel> = AcceptableItem;
        let allItems : Array<AcceptableItemModel> = await document.find();
        let array: Array<any> =  allItems.map(item => sendItemInfo(item));
